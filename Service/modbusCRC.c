...
unsigned int ModBus_CalCRC16(const unsigned char *dataIn, unsigned int length)
{
	...
	for (i = 0; i < length; i++)
	{
		crc = (crc >> 8) ^ crc16_table[(crc ^ dataIn[i]) & 0xFF];
	}
	return crc; // 返回校验值（低位在前）
}
...
unsigned int ModBus_CalCRC16(const unsigned char *dataIn, unsigned int length)
{
	...
	for (i = 0; i < length; i++)
	{
		crc ^= dataIn[i]; // 当前字节与CRC低字节异或
		for (j = 0; j < 8; j++)
		{
			if (crc & 0x0001) // 检查最低位
			{
				crc >>= 1;	   // 右移1位
				crc ^= 0xA001; // 多项式 0x8005 的反转（0xA001）
			}
			else
			{
				crc >>= 1; // 直接右移
...
unsigned char CheckArray(unsigned char *dataIn, unsigned int length)
{
	...
	crcCal = ModBus_CalCRC16(dataIn, length - 2);
	crcIn = dataIn[length - 1] << 8;
	crcIn = crcIn | dataIn[length - 2];
...
unsigned char ArrayAddCheck(unsigned char *dataIn, unsigned int length)
{
	...
	crcCal = ModBus_CalCRC16(dataIn, length - 2);
	dataIn[length - 1] = crcCal >> 8;
	dataIn[length - 2] = crcCal & 0xFF;
